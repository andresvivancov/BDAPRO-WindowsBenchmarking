...
class TwitterStreamGenerator(filterTerms: String*) extends SourceFunction[Tweet]{
  ...
  override def run(ctx: SourceContext[Tweet]): Unit = {
    ...
    cb.setDebugEnabled(true)
      .setOAuthConsumerKey("X0k4zycdWOVU7gLUdiU")
      .setOAuthConsumerSecret("nq42UJ75jVhYTRfHFdanXOj9qreAvP5OzaWw6Gt38MGnU")
      .setOAuthAccessToken("1206314257-gPBrxJIry5pXkenRlUW")
      .setOAuthAccessTokenSecret("UM262OPTvNQ4I1ZqLWAKMgR86a8GTVJDcM")
...
object TwitterStreamDemo {
  ...
  //  val values = tweets.flatMap(x => x.userName)

    val keyValue = tweets.keyBy(x => x.userName)
