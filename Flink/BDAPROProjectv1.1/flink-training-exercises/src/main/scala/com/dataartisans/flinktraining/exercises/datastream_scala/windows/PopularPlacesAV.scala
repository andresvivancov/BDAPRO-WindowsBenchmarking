...
object PopularPlacesAV {
  ...
  //  val params = ParameterTool.fromArgs(args)
   // val input = params.getRequired("input")
    ...
    val rides = env.addSource(new TaxiRideSource("/home/andresviv/Temp/nycTaxiRides.gz", maxDelay, speed))
  ...
  class GridToCoordinates extends MapFunction[
    ...
    (Float, Float, Long, Boolean, Int, Long)] {

    def map(cellCount: (Int, Long, Boolean, Int)): (Float, Float, Long, Boolean, Int,Long) = {
      val longitude = GeoUtils.getGridCellCenterLon(cellCount._1)
      val latitude = GeoUtils.getGridCellCenterLat(cellCount._1)
      (longitude, latitude, cellCount._2, cellCount._3, cellCount._4,cellCount._2)
    }
  }
