...
object ReadWordCountWindow5 {
 ...
 //   val text = env.readTextFile("/home/andresviv/Temp/big.txt");
    ...
    val text = env.readFile(inp,
                ...
                300l,FilePathFilter.createDefaultFilter());
    /**
      * its better to use
    ...
    val counts = text.flatMap { _.toLowerCase.split("\\W+") filter { _.nonEmpty } }
     .map { (_,System.currentTimeMillis, 1) }
      //  .map(parseMap(_))
    val timedValue = counts.assignAscendingTimestamps(x => x._2)

    val keyValuePair = timedValue.keyBy(0).timeWindow(Time.milliseconds(100))

    val countStream = keyValuePair.sum(2)
